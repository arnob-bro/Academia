...
export const FacultyManagement = () => {
  ...
  const [facultyData, setFacultyData] = useState([
    { id: 'F001', name: 'John Doe', department: 'Computer Science', courses: ['CS101', 'CS102'], role: 'admin' },
    { id: 'F002', name: 'Jane Smith', department: 'Mathematics', courses: ['MATH101'], role: 'N/A' },
    { id: 'F003', name: 'Jack Doe', department: 'Computer Science', courses: ['CS101', 'CS102'], role: 'admin' },
    { id: 'F004', name: 'Meow Meow', department: 'Computer Science', courses: ['CS101', 'CS102'], role: 'admin' },
  ]);
  ...
  const filteredFaculty = facultyData.filter(faculty => {
    return faculty[filterType].toLowerCase().includes(searchQuery.toLowerCase());
  });
  ...
  const saveSelectedCourses = () => {
    setFacultyData((prevData) =>
      prevData.map((faculty) =>
        faculty.id === selectedFaculty.id ? { ...faculty, courses: selectedCourses } : faculty
      )
    );
            ...
            {filteredFaculty.map((faculty) => (
              <tr key={faculty.id}>
                ...
                <td>
                  ...
                  <button className='faculty-management-select-course-button' onClick={() => openModal(faculty)}>
                    Select Courses
                  </button>
                </td>
                <td>
                  <select onChange={(e) => 
                    setFacultyData(prevData => prevData.map(fac => fac.id === faculty.id ? { ...fac, role: e.target.value } : fac))} 
                    value={faculty.role}>
